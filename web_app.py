...
def get_grade_info(score_input):
...
with st.form("grade_form", clear_on_submit=True):
    ...
    with col1:
        course_name = st.text_input("Course Name (e.g., BCAD 103)")
    with col2:
        score_val = st.number_input("Score", min_value=0, max_value=100, step=1)
    
    submit = st.form_submit_button("Add to My Records")

# This is Line 39 where the error was happening
if submit and course_name:
    grade_letter, grade_point = get_grade_info(score_val)
    new_row = pd.DataFrame({'Course': [course_name], 'Score': [score_val], 'Grade': [grade_letter], 'GP': [grade_point]})
...
if not st.session_state.df.empty:
    ...
    total_courses_count = len(st.session_state.df) + remaining_courses
    required_total_gp = (target_gpa * total_courses_count) - current_total_gp
    ...
    # 6. PDF Generation
    def generate_pdf(report_df, final_cgpa):
        pdf = FPDF()
        pdf.add_page()
        ...
        pdf.ln(10)
        pdf.cell(200, 10, txt=f"Final CGPA: {final_cgpa:.2f}", ln=True)
